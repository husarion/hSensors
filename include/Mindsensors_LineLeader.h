...
public:
	...
	/**
	* @brief Read the uncalibrated sensor values from the Line Leader.
	...
	* @param *pTab is 8 bytes returned.  One for each sensor.
	* @return error code
	*/
	EError  readSensorRaw(uint8_t *pTab);

	/**
	* @brief Read an uint8_t value with each bit equal to a sensor.
	* 1 = Line
	* 0 = No Line
	...
	*/
	...
	/**
	*@Brief This function reads the setpoint value from the sensor.
	* Setpoint is a point where should be center of the line.
	* @param &v is a one byte returned.
	* @return error code
	*/
	...
	* reads Kp value without a factor. p = Kp / KpFactor
	...
	* This function reads Ki value without a factor. i = Ki / KiFactor
	...
	* This function reads Kd value without a factor. d = Kd / KdFactor
	* @param &v returned value.
	* @return error code
	*/
	...
	*/
	...
	/**
	* @Brief Read the "White Threshold" values from the Line Leader for each sensor.<br>
